...
public class GestoreIO {
	...
	//Metodi per stampare affianco
	
	@SuppressWarnings("resource")
	public String leggiStringaContinua(String messaggio) {
		System.out.println(messaggio);
		Scanner input=new Scanner(System.in);
		return input.nextLine();
	}
	...
	public Date leggiDateContinua(String messaggio) {
		System.out.println(messaggio);
		int anno= leggiInt("Inserisci l'anno: ");
		int mese=leggiInt("Inserisci il mese: ");
		int giorno= leggiInt("Inserisci il giorno: ");
	...
	public void formPersona(Persona newPers) {
		...
		if (newPers instanceof Dipendente) {
			((Dipendente) newPers).setStipendio(leggiDouble("Inserisci lo stipendio: "));
			if (newPers instanceof Manager) {
				((Manager) newPers).setAreaResp(leggiStringa("Inserisci l'area di responsabilità: "));
			}
		}
	}
	...
	public boolean duplica(Persona personaOriginale) {
		if (personaOriginale!=null) {
			Persona personaDuplicata=personaOriginale;
			...
			if (personaOriginale instanceof Dipendente) {
				...
				if (personaOriginale instanceof Manager) {
					((Manager)personaDuplicata).setAreaResp(((Manager)personaOriginale).getAreaResp());
				}
			}
			return true;
		}
	...
	public void formModifica(Persona newPers) {
		newPers.setNome(leggiStringaContinua("NOME: "+newPers.getNome()+"  ---> "));
		newPers.setCognome(leggiStringaContinua("COGNOME: "+newPers.getCognome()+" ---> "));
		newPers.setDataDiNascita(leggiDateContinua("DATA DI NASCITA "+newPers.getDataDiNascita()+" ---> "));
		if (newPers instanceof Dipendente) {
			((Dipendente) newPers).setStipendio((leggiDoubleContinua("STIPENDIO: "+(((Dipendente)newPers).getStipendio())+" --->")));
			if (newPers instanceof Manager) {
				((Manager) newPers).setAreaResp(leggiStringaContinua("AREA DI RESPONSABILITÀ: "+(((Manager)newPers).getAreaResp())+" --->"));
			}
		}
	}
}
